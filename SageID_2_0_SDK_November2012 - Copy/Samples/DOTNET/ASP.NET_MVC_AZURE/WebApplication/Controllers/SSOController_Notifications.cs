...
namespace WebApplication.Controllers
{
    public partial class SSOController : Controller
    {
        ...
        public ActionResult SSONotify()
        {
            try
            {
                // Abort if the notification is too large
                if (Request.InputStream.Length > MaxNotificationSize)
                {
                    throw new Exception("Notification exceeds maximum allowed size.");
                }

                // Notification data is supplied as the body of the POST
        ...
        private bool IsReplayedId(Guid id)
        {
            ...
            if (HttpContext.Cache.Get(idKey) == null)
            {
                HttpContext.Cache.Add(idKey,                      // The Notification ID
